...
internal class FragivityEffectsController(container: ViewGroup) :
    ...
    override fun executeOperations(operations: List<Operation>, isPop: Boolean) {
        ...
        var firstOut: Operation? = null
        var lastIn: Operation? = null
        for (operation: Operation in operations) {
            val currentState = Operation.State.from(operation.fragment.mView)
            when (operation.finalState) {
                Operation.State.GONE, Operation.State.INVISIBLE, Operation.State.REMOVED -> if (currentState == Operation.State.VISIBLE && firstOut == null) {
                    // The firstOut Operation is the first Operation moving from VISIBLE
                    firstOut = operation
                }
                Operation.State.VISIBLE -> if (currentState != Operation.State.VISIBLE) {
        ...
        if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
            Log.v(
                FragmentManager.TAG,
                "Executing operations from $firstOut to $lastIn"
            )
        }

        // Now iterate through the operations, collecting the set of animations
        // and transitions that need to be executed
        val animations: MutableList<AnimationInfo> = ArrayList()
        val transitions: MutableList<TransitionInfo> = ArrayList()
        val awaitingContainerChanges: MutableList<Operation> = ArrayList(operations)
        ...
        for (operation: Operation in operations) {
            ...
            transitions.add(
                TransitionInfo(
                    operation, transitionCancellationSignal, isPop,
        ...
        val startedTransitions = startTransitions(
            transitions,
        ...
        startAnimations(
            animations, awaitingContainerChanges,
    ...
    private fun syncAnimations(operations: List<Operation>) {
        ...
        val lastOpFragment = operations[operations.size - 1].fragment
    ...
    private fun startAnimations(
        ...
        startedAnyTransition: Boolean, startedTransitions: Map<Operation, Boolean>
    ) {
        val container = container
        val context = container.context
        val animationsToRun = ArrayList<AnimationInfo>()
        ...
        for (animationInfo: AnimationInfo in animationInfos) {
            ...
            val startedTransition = java.lang.Boolean.TRUE == startedTransitions[operation]
            if (startedTransition) {
                if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
                    Log.v(
                        FragmentManager.TAG, "Ignoring Animator set on "
            ...
            animator.addListener(object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(anim: Animator) {
                    ...
                    if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
                        Log.v(
                            FragmentManager.TAG, "Animator from operation " + operation + " has "
                                    + "ended."
                        )
                    }
                }
            ...
            signal.setOnCancelListener {
                ...
                if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
                    Log.v(
                        FragmentManager.TAG,
                        ("Animator from operation " + operation + " has "
                                + "been canceled.")
                    )
                }
            }
        ...
        for (animationInfo: AnimationInfo in animationsToRun) {
            ...
            if (startedAnyTransition) {
                if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
                    Log.v(
                        FragmentManager.TAG, "Ignoring Animation set on "
...
//                continue
            ...
            val viewToAnimate = fragment.mView
            val anim = Preconditions.checkNotNull(
            ...
            if (finalState != Operation.State.REMOVED) {
                ...
                container.startViewTransition(viewToAnimate)
                val animation: Animation = EndViewTransitionAnimation(
                ...
                animation.setAnimationListener(object : Animation.AnimationListener {
                    override fun onAnimationStart(animation: Animation) {
                        if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
                            Log.v(
                                FragmentManager.TAG, "Animation from operation " + operation
            ...
            signal.setOnCancelListener {
                ...
                if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
                    Log.v(
                        FragmentManager.TAG,
                        ("Animation from operation " + operation
                                + " has been cancelled.")
                    )
                }
            }
    ...
    private fun startTransitions(
        ...
        awaitingContainerChanges: MutableList<Operation>,
        isPop: Boolean, firstOut: Operation?,
        lastIn: Operation?
    ): Map<Operation, Boolean> {
        val startedTransitions: MutableMap<Operation, Boolean> = HashMap()
        // First verify that we can run all transitions together
        var transitionImpl: FragmentTransitionImpl? = null
        for (transitionInfo: TransitionInfo in transitionInfos) {
            ...
            if (transitionImpl == null) {
                ...
                throw IllegalArgumentException(
                    "Mixing framework transitions and "
                            + "AndroidX transitions is not allowed. Fragment "
                            + transitionInfo.operation.getFragment() + " returned Transition "
            ...
            if (hasSharedElementTransition && (firstOut != null) && (lastIn != null)) {
                ...
                sharedElementTransition = transitionImpl.wrapTransitionInSet(
                    transitionImpl.cloneTransition(
                        transitionInfo.sharedElementTransition
                    )
                )
                // The exiting shared elements default to the source names from the
                // last in fragment
                val exitingNames = lastIn.fragment
                ...
                val firstOutSourceNames = firstOut.fragment
                    .sharedElementSourceNames
                val firstOutTargetNames = firstOut.fragment
                ...
                val enteringNames = lastIn.fragment
                    .sharedElementTargetNames
                var exitingCallback: SharedElementCallback?
                var enteringCallback: SharedElementCallback?
                if (!isPop) {
                    // Forward transitions have firstOut fragment exiting and the
                    // lastIn fragment entering
                    exitingCallback = firstOut.fragment.exitTransitionCallback
                    enteringCallback = lastIn.fragment.enterTransitionCallback
                } else {
                    // A pop is the reverse: the firstOut fragment is entering and the
                    // lastIn fragment is exiting
                    exitingCallback = firstOut.fragment.enterTransitionCallback
                    enteringCallback = lastIn.fragment.exitTransitionCallback
                }
                ...
                if (enteringCallback != null) {
                    ...
                    for (i in enteringNames.indices.reversed()) {
                        ...
                        if (view == null) {
                            val key = FragmentTransition.findKeyForValue(
                ...
                retainMatchingViews(firstOutViews, sharedElementNameMapping.keys)
                ...
                if (sharedElementNameMapping.isEmpty) {
                    ...
                    FragmentTransition.callSharedElementStartEnd(
                        lastIn.fragment, firstOut.fragment, isPop,
                        firstOutViews, true
                    )
        ...
        mergedTransition = transitionImpl.mergeTransitionsInSequence(
            mergedTransition,
            ...
            if (transition != null || involvedInSharedElementTransition) {
                ...
                if (!ViewCompat.isLaidOut(container)) {
                    if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
                        Log.v(
                            FragmentManager.TAG,
                            ("SpecialEffectsController: Container " + container
                    ...
                    transitionImpl.setListenerForTransitionEnd(
                        transitionInfo.operation.getFragment(),
                        mergedTransition,
        ...
        if (FragmentManager.isLoggingEnabled(Log.VERBOSE)) {
            ...
            for (view: View in sharedElementLastInViews) {
                Log.v(
                    FragmentManager.TAG, ("View: " + view + " Name: "
        ...
        transitionImpl.setNameOverridesReordered(
            container, sharedElementFirstOutViews,
     ...
     */
    private fun retainMatchingViews(
        sharedElementViews: ArrayMap<String?, View>,
        transitionNames: Collection<String?>
    ) {
        val iterator: MutableIterator<Map.Entry<String?, View>> =
            sharedElementViews.entries.iterator()
        while (iterator.hasNext()) {
            val entry = iterator.next()
            if (!transitionNames.contains(ViewCompat.getTransitionName(entry.value))) {
                iterator.remove()
            }
        }
    }
    ...
    fun  /* synthetic access */applyContainerChanges(operation: Operation) {
    ...
    private open class SpecialEffectsInfo(
        ...
        val isVisibilityUnchanged: Boolean
            get() {
                val currentState = Operation.State.from(
                    operation.fragment.mView
                )
                val finalState = operation.finalState
    ...
    private class AnimationInfo(
        operation: Operation, signal: CancellationSignal,
        private val mIsPop: Boolean
    ) :
        SpecialEffectsInfo(operation, signal) {
        private var mLoadedAnim = false
        private var mAnimation: AnimationOrAnimator? = null
        fun getAnimation(context: Context): AnimationOrAnimator? {
            if (mLoadedAnim) {
                return mAnimation
            }
            mAnimation = FragmentAnim.loadAnimation(
                context,
                operation.fragment,
    ...
    private class TransitionInfo(
        operation: Operation,
        signal: CancellationSignal, isPop: Boolean,
        ...
        init {
            if (operation.finalState == Operation.State.VISIBLE) {
                ...
                isOverlapAllowed =
                    if (isPop) operation.fragment.allowReturnTransitionOverlap else operation.fragment.allowEnterTransitionOverlap
            } else {
                transition =
            ...
            if (providesSharedElementTransition) {
                if (isPop) {
                    sharedElementTransition = operation.fragment.sharedElementReturnTransition
                } else {
                    sharedElementTransition = operation.fragment.sharedElementEnterTransition
                }
            } else {
                sharedElementTransition = null
            }
            ...
            get() {
                ...
                if ((transitionImpl != null) && (sharedElementTransitionImpl != null
                    ...
                    throw IllegalArgumentException(
                        ("Mixing framework transitions and "
                                + "AndroidX transitions is not allowed. Fragment "
                                + operation.fragment + " returned Transition "
                                + transition + " which uses a different Transition "
        ...
        private fun getHandlingImpl(transition: Any?): FragmentTransitionImpl? {
            ...
            throw IllegalArgumentException(
                ("Transition " + transition + " for fragment "
                        + operation.getFragment() + " is not a valid framework Transition or "
                        + "AndroidX Transition")
            )
        }
    }
